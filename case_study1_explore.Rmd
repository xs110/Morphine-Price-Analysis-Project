...
```
```{r}
histogram(~log_ppm|quarter,data = df)
```
...
```{r}
fit <- with(imp, anova(lmer(log_ppm ~  year + mgstr * bulk_purchase +  source + (1+ year|state)),
                               lmer(log_ppm ~ year + mgstr *mgstr  + bulk_purchase + source + (1|state))))
#AIC
mean(summary(fit)$AIC[c(1,3,5,7,9)])
mean(summary(fit)$AIC[c(2,4,6,8,10)])
